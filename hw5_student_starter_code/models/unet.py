...
class UNet(nn.Module):
    ...
    # input_size: spatial size of inputs (not strictly required here, but kept for sanity).
    # input_ch: input/output channels (e.g., 3 for RGB or 4 for latent channels).
    # T: total number of diffusion timesteps (for embedding).
    # ch: base channel width (model size).
    # ch_mult: list like [1,2,2,4] telling how channels grow as we go down.
    # attn: which resolution stages use attention (list of indices).
    # num_res_blocks: how many ResBlocks per stage.
    # dropout: dropout rate inside blocks.
    # conditional: whether to use cross-attention with external condition c.
    # c_dim: dimensionality of the condition (needed if conditional=True).
