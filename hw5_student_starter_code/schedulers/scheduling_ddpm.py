...
class DDPMScheduler(nn.Module):
    ...
    def __init__(
        ...
        if self.beta_schedule == 'linear':
            ...
            # Calculate alphas_cumprod using the cosine formula
        ...
        # TODO: timesteps
        timesteps = torch.arange(self.num_train_timesteps, dtype=torch.float32)
    ...
    def set_timesteps(
        ...
        #timesteps = np.linspace(0, self.num_train_timesteps - 1, num_inference_steps, dtype=np.int32)
        timesteps = np.linspace(self.num_train_timesteps - 1, 0, num_inference_steps, dtype=np.int32)
    ...
    def previous_timestep(self, timestep):
        ...
        prev_t = max(0, timestep - (self.num_train_timesteps // num_inference_steps))
        return prev_t
    ...
    def _get_variance(self, t):
        ...
        alpha_prod_t_prev =  self.alphas_cumprod[prev_t] if t > 0 else torch.tensor(1.0)
        ...
        if self.variance_type == "fixed_small":
            ...
            variance = current_beta_t * (alpha_prod_t_prev / alpha_prod_t)
