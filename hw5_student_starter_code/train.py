...
def parse_args():
    ...
    #EMA
    parser.add_argument("--use_ema", type=str2bool, default=False, help="use ema to smooth training")

    parser.add_argument("--resume", type=str2bool, default=False, help="Resume training")
    parser.add_argument("--resume_ckpt", type=str, default=None, help="checkpoint path for resume training")
...
def main():
    ...
    unet = UNet(
        ...
        conditional=args.use_cfg,  # <- force OFF cross-attention / CFG
        c_dim=args.unet_ch  # <- no class embedding dim
    )
    ...
    class_embedder = None
    if args.use_cfg:
        # TODO: class_embedder = ClassEmbedder(None)
        class_embedder = ClassEmbedder(embed_dim=args.unet_ch, n_classes=args.num_classes).to(device)
    ...
    scheduler_optim = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.num_epochs, eta_min=1e-6)
    ...
    if args.use_ema:
        ...
        ema = EMA(unet_wo_ddp, beta=0.995)
    
    # TODO: setup ddim
    if args.use_ddim:
        scheduler_wo_ddp = DDIMScheduler(beta_start=args.beta_start,
                              beta_end=args.beta_end,
                              num_train_timesteps=args.num_train_timesteps,
                              num_inference_steps = args.num_inference_steps,
                              beta_schedule = args.beta_schedule,
    ...
    start_epoch = 0
    if args.resume:
        logger.info(f"Resuming from {args.resume_ckpt}") 
        start_epoch = load_checkpoint(
            ...
            optimizer,
            lr_scheduler=scheduler_optim,
            ema=ema,
            checkpoint_path=args.resume_ckpt
            )
        if scheduler_optim.last_epoch == -1 and start_epoch > 0:
            logger.info(f"Forcing scheduler to epoch {start_epoch}")
            for _ in range(start_epoch):
                scheduler_optim.step()
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if is_primary(args):
            ...
            save_checkpoint(unet_wo_ddp, scheduler_wo_ddp, vae_wo_ddp, class_embedder, optimizer, epoch, ema=ema, lr_scheduler=scheduler_optim, save_dir=save_dir)
